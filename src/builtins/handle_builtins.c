...
int	handle_builtins(t_ms *ms) //probably has to be **msh to do exil propery and equal pointer to null
{
	...
	if (msh->args_list == NULL) // only stays here to check bad redirection
	...
	if (!ft_strncmp(args_list->arg, "env", 3) && ft_strlen(args_list->arg) == 3)
		ms->exec_value = ft_env(ms->env); //done, but needs checker
	if (!ft_strncmp(args_list->arg, "export", 6) && ft_strlen(args_list->arg) == 6)
		return (ft_export()); 
	if (!ft_strncmp(args_list->arg, "unset", 5) && ft_strlen(args_list->arg) == 5)
		return (ft_unset(env_list, args_list)); //probably the &env_list
	if (!ft_strncmp(args_list->arg, "exit", 4) && ft_strlen(args_list->arg) == 4)
